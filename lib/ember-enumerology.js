...
(function() {
  Enumerology.Filter = Em.Mixin.create({
    initialValue: [],
    isReduce: false,
    isFilter: true
  });
...
(function() {
  ...
  pipeline = Em.Object.extend({
    ...
    finalize: function() {
      ...
      return Ember.reduceComputed.apply(Ember, [baseKey].concat(__slice.call(dependentKeys), [{
        ...
        initialize: function(initialValue, changeMeta, instanceMeta) {
          ...
          return transformations.forEach(function(transformation, index) {
            return instanceMeta.transformationAccumulators[index] = transformation.get('initialValue');
          });
        },
        addedItem: function(accumulator, item, changeMeta, instanceMeta) {
          var context, returnValue;
          context = changeMeta;
          context.binding = this;
          returnValue = accumulator;
          transformations.forEach(function(transformation, index) {
            var accumulatedValue;
            accumulatedValue = instanceMeta.transformationAccumulators[index];
            return returnValue = transformation.addedItem(accumulatedValue, item, context);
          });
          return returnValue;
        },
        removedItem: function(accumulator, item, changeMeta, instanceMeta) {
          ...
          transformations.forEach(function(transformation, index) {
            var accumulatedValue;
            accumulatedValue = instanceMeta.transformationAccumulators[index];
            return returnValue = transformation.removedItem(accumulatedValue, item, context);
          });
          return returnValue;
        }
      }]));
  ...
  any = Enumerology.Transform.extend(Enumerology.Reduce, {
    ...
    addedItem: function(accumulatedValue, item, context) {
      ...
      if (callback.call(context.binding, item)) {
        this.incrementProperty('trueCount');
      }
    ...
    removedItem: function(accumulatedValue, item, context) {
      ...
      callback = this.get('callback');
      if (callback.call(context.binding, item)) {
        this.decrementProperty('trueCount');
      }
      return this.get('value');
    }
  });
...
(function() {
  ...
  filter = Enumerology.Transform.extend(Enumerology.Filter, {
    ...
    init: function() {
      ...
      return this.set('initialValue', Ember.A());
    },
